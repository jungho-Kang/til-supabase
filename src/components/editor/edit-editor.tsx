import { BlogRow, updateBlogId } from "@/app/actions/blog-action";
...
const EditEditor = ({ blog }: { blog: BlogRow }) => {
  ...
  // 서버에 저장된 이미지 관리하는 법
  // 1. 삭제 예정 이미지 URL을 저장하는 배열
  const [imagesToDelete, setImagesToDelete] = useState<string[]>([]);

  // 2. 새로이 업로드 되는 이미지 URL을 저장하는 배열
  const [newUploadImages, setNewUploadImages] = useState<string[]>([]);

  // 3. 기존 내용에 포함된 Image URL을 추출함
  ...
  const fetchUpdateBlog = async () => {
    // 1. 업데이트 하기 전에 삭제 이미지들은 삭제 처리부터 진행
    // 반복문으로 삭제 실행
